...
data_path <- "../dat/raw_data/Bulk_searches/Mon_and_Mel.tsv" 
...
t7_Monocytes <- as.data.frame(t7) %>% dplyr::select(intersect(colnames(t7),meta_mon$id))
t7_Melanomas <- as.data.frame(t7) %>% dplyr::select(intersect(colnames(t7),meta_mel$id))

t7_Melanomas_Large <- as.data.frame(t7) %>% dplyr::select(intersect(colnames(t7),meta_mel_C1$id))
t7_Melanomas_Small <- as.data.frame(t7) %>% dplyr::select(intersect(colnames(t7),meta_mel_C2$id))
...
Mon_DNA <- ggplot(FCS_Mon,aes(x =DNA_Content, fill =  'DNA_Content')) + geom_histogram(bins = 100,alpha = .3) + 
  xlim(c(400000,1.56*10^6))+theme_classic()+ scale_fill_manual(values = '#007A39')+ 
  ylab('# of cells') + xlab('DNA content')+rremove('legend')+
  theme(axis.title=element_text(size=36),text=element_text(size=0))

Mel_DNA <- ggplot(FCS_Mel,aes(x =DNA_Content, fill =  'DNA_Content')) + geom_histogram(bins = 100,alpha = .3) + 
...
In this step were are looking at bulk samples of cells sorted by FACS to find proteins whose abundance is up-regulated in G1, S, or G2 phase of the cell cycle. We will use these proteins for our single cell analysis to find functions associated with each phase, and determine each cells position in the cell cycle. To further narrow list of proteins that will be considered for downstream analysis, we incorporate additional constraints like requiring multiple peptides for each protein in our single cells who's peptides positively correlate.  
```{r Bulk CDC Markers}

#Annotations for different runs of bulk CDC fraction data
...
evi <- evi %>% filter(Run != 'wAL00161')

evi_prot1 <- diann_maxlfq(evi,sample.header = "File.Name",group.header = "Protein.Group",id.header = "Precursor.Id",quantity.header = "Ms1.Area")
evi_prot1 <- filt.mat.rc(evi_prot1,.1,.1)
evi_prot2 <- diann_maxlfq(evi,sample.header = "File.Name",group.header = "Protein.Group",id.header = "Precursor.Id",quantity.header = "PG.Quantity")
evi_prot2 <- filt.mat.rc(evi_prot2,.1,.1)

sect <- intersect(rownames(evi_prot2),rownames(evi_prot1))

evi_prot1<- evi_prot1[sect,]
evi_prot2<- evi_prot2[sect,]
...
evi_mels <- diann::diann_maxlfq(evi_mels, group.header="Protein.Group", id.header = "Precursor.Id", quantity.header = "Ms1.Area")
for(i in 1:ncol(evi_mels)){
  evi_mels[,i] <-evi_mels[,i]/median(evi_mels[,i],na.rm = T)
}
for(i in 1:nrow(evi_mels)){
  evi_mels[i,] <- evi_mels[i,]/mean(evi_mels[i,],na.rm = T)
}
evi_mels <- log2(evi_mels)

look <- as.data.frame(evi_mels) %>% filter(rownames(evi_mels) %in% c('P06493','P11388','O00762','P62805'))
...
for (i in 1:length(unique(evi_all$Protein.Group))){
  ...
  if(nrow(evi_hold) > 5){
    if(length(unique(evi_hold$Condition)) > 1){
        ...
        evi_hold <- suppressMessages(evi_hold %>% group_by(variable,Protein.Group) %>% dplyr::summarize(value = median(value,na.rm = T)))
...
write.csv(diff_prot_output_filt,'/Users/andrewleduc/Desktop/nPOP_Paper/dat/Output_tables/Bulk_CDC_diff_prot.csv')
...
G1_df <- G1_df %>% filter(Protein %in% rownames(t7_filt_mel))
G1_df_final <- G1_df[1:15,]


S_df <- diff_prot_output_filt %>% filter(S > G2)
S_df <- S_df %>% filter(S > G1)
S_df <- S_df[order(-S_df$S),]
S_df <- S_df %>% filter(Protein %in% rownames(t7_filt_mel))
S_df_final <- S_df[1:15,]


G2_df <- diff_prot_output_filt %>% filter(G2 > S)
G2_df <- G2_df %>% filter(G2 > G1)
G2_df <- G2_df[order(-G2_df$G2),]
G2_df <- G2_df %>% filter(Protein %in% rownames(t7_filt_mel))
G2_df_final <- G2_df[1:15,]
nrow(t7_Melanomas)
...
#write(G1_prots,'../dat/Proccessed_data/G1_prots_new.txt')
#write(S_prots,'../dat/Proccessed_data/S_prots_new.txt')
#write(G2_prots,'../dat/Proccessed_data/G2_prots_new.txt')


```


To average out noise in our single cell measurements, here we construct protein "Marker" vectors for each phase of the cell cycle. Protin marker vectors are the vectors of averaged abundance of 2 or 3 proteins. Here we search for different protein markers that positivly correlate and store pairs of markers that pass a certain arbitrary significance threshold. This is a huge for loop that takes a while to run (sorry) that tests the correlations between all possible combinations of 2 or 3 proteins.
```{r Phase Marker Construction}
G1_prots <- read.delim('../dat/Proccessed_data/G1_prots.txt',header = F)
G1_prots <- G1_prots$V1
S_prots <- read.delim('../dat/Proccessed_data/S_prots.txt',header = F)
S_prots <- S_prots$V1
G2_prots <- read.delim('../dat/Proccessed_data/G2_prots.txt',header = F)
G2_prots<- G2_prots$V1
...
CC_sub <- list(G1_sub,S_sub,G2_sub)
    ...
    for (k in 1:(length(choose2[1,]))){
      #Making sure two paired phase markers dont contain same proteins
       if (length(intersect(choose[,choose2[1,k]],choose[,choose2[2,k]])) == 0){
          mark1 <- colMeans(phase[choose[,choose2[1,k]],],na.rm = T)
          prot1 <- rownames(phase[choose[,choose2[1,k]],])
          mark2 <- colMeans(phase[choose[,choose2[2,k]],],na.rm = T)
          prot2 <- rownames(phase[choose[,choose2[2,k]],])
        ...
        if(mark_cor > .2){
          if(i == 2){
            ...
            if(sig < .00000001){
              ...
              CC_fill_sub1[[i]][(2*count2),] <- colMeans(phase[choose[,choose2[1,k]],],na.rm = T)
              prot1 <- paste0(prot1[1],'-',prot1[2],'-',prot1[3])
              CC_fill_sub1[[i]][(2*count2-1),] <- colMeans(phase[choose[,choose2[2,k]],],na.rm = T)
...
length(prot_L) == (nrow(mG1)+nrow(mS)+nrow(mG2))

```

Once we have our protein marker vectors, we construct a matrix of 6 marker vectors (2 from each phase) so that the correlations out of phase are negative. We then validate the correlations of these proteins by examine if the correlation matricies of the same proteins have the same structure within the melanoma cells. We record all different possible marker combinations that work.
```{r Find marker combinations that anti-correlate out of phases}
a_hold <- 1
mat_dif_hold <- 0
mark_mat <- matrix(data = NA,nrow = 6,ncol = ncol(G1_sub))
#mark_mat_new <- matrix(data = NA,nrow = 6,ncol = ncol(G1_sub))
dist_stor <- c()
its <- 0
i_t <- c()
j_t <- c()
k_t <- c()
s_store <- c()
#a <- (cor(t(mG1),use = 'pairwise.complete.obs'))
s3_hold <-c()
for(i in 1:(nrow(mG1)/2)){
  ...
  for (j in 1:(nrow(mS)/2)){
    ...
    for (k in 1:(nrow(mG2)/2)){
      ...
      s1 <- median(c(a[2:3],a[6:7]))
      s2 <- median(c(a[4:5],a[8:9]))
      s3 <- median(a[11:14])
      ...
      s3_hold <-c(s3_hold,s1)
      
      if((s1 < -.02) & (s2 < -.03) & (s3 < -.03)){
          i_t <- c(i_t,i)
          j_t <- c(j_t,j)
          k_t <- c(k_t,k)
          print('here')
...
The histogram shows the concordance between the correlation structure of the different possible protein markers combinations that meet conditions (correlate positive to same phase, negative to different phase) in the melanoma cells and the monocytes. This confirms that these trends exist in the melanoma cells independent of the construction procedure used to formulate the marker combinations. 
...
for(i in 1:9){
  ...
  print(cor.test(mark_mat_Mel_vect,mark_mat_Mon_vect)$p.value)
  #if(mark_mat_Mel_vect[10] > 0){
   # if((mark_mat_Mel_vect[11] < 0)&(mark_mat_Mel_vect[12] < 0)&(mark_mat_Mel_vect[13] < 0)&(mark_mat_Mel_vect[14] < 0)){
    #  if(cor(mark_mat_Mel_vect,mark_mat_Mon_vect,use = 'pairwise.complete.obs',method = 'pearson') > cor_stor){
     #   neg_stor <- median(mark_mat_Mel_vect[11:14])
      #  cor_stor <- cor(mark_mat_Mel_vect,mark_mat_Mon_vect,use = 'pairwise.complete.obs',method = 'pearson')
      #  i_hold <- i
     #   print('here')
     # }
    #}
...
mean(real$Correlations,na.rm = T)



```

Here we correlate all proteins to the protein markers and look for significant proteins via spearman test p-value
```{r Find New Prots that correlate with CDC markers}
t7_Monocytes <- as.data.frame(filt.mat.rc(as.matrix(t7_Monocytes),.9,.95))
t7_Melanomas <- as.data.frame(filt.mat.rc(as.matrix(t7_Melanomas),.9,.95))
t7_Monocytes <- t7_Monocytes %>% filter(rownames(t7_Monocytes) %in% rownames(t7_Melanomas))
t7_Melanomas <- t7_Melanomas %>% filter(rownames(t7_Melanomas) %in% rownames(t7_Monocytes))
...
for (i in 1:nrow(t7_Monocytes)){
  ...
  CDC_prot_disco_mon[count,4] <- cor(t(t7_Monocytes[i,]),U_markG2[[3]],use = 'pairwise.complete.obs')
  CDC_prot_disco_mel[count,4] <- cor(t(t7_Melanomas[i,]),M_markG2[[3]],use = 'pairwise.complete.obs')
...
CDC_prot_disco_mon <- CDC_prot_disco_mon %>% filter(qval < .05)
CDC_prot_disco_mel <- CDC_prot_disco_mel %>% filter(qval < .05)

CDC_prot_disco_mon <- CDC_prot_disco_mon %>% filter(cor > 0)
CDC_prot_disco_mel <- CDC_prot_disco_mel %>% filter(cor > 0)

nrow(CDC_prot_disco_mel)
nrow(CDC_prot_disco_mon)

CDC_prot_disco_mel$celltype <- 'Melanoma'
CDC_prot_disco_mon$celltype <- 'Monocyte'
CDC_prot_disco<- rbind(CDC_prot_disco_mel,CDC_prot_disco_mon)
#Mel S, Q07021,Q13561,Q15118,Q9UBE0 G1, Q9H307,Q16778,P06748,Q14978, G2 'O14818
#mon g2 O43683
CDC_prot_disco_sect <- CDC_prot_disco %>% filter(prot %in% intersect(CDC_prot_disco_mel$prot,CDC_prot_disco_mon$prot))
#P06748

write.csv(CDC_prot_disco,'/Users/andrewleduc/Desktop/nPOP_Paper/dat/Output_tables/CDC_protein_correlations.csv')
```

Here we are doing a protein set enrichment analysis where we compute the correlation of all proteins in a gene set to each of the cell cycle maker vectors. This gives us 3 distributions of correlations, correlations of proteins to G1, S, and G2, which we then compute significance with ANOVA. This analysis is computed with all different protein marker combinations and GO terms coming up as significant across many different marker combinations serves as validation.
```{r Protein Set Enrichment analysis All}

#filtering protein matricies for monocytes and melanoma 
...
sum(is.na(all_prot_U)==F)/(nrow(all_prot_U)*ncol(all_prot_U))
sum(is.na(all_prot_M)==F)/(nrow(all_prot_U)*ncol(all_prot_M))
...
# Taking not of how frequently same GO terms are found significant
...
#all_go_count <- all_go_count %>% filter(V2 > 9)
...
#store correlations for all Go terms that meet criteria 
#for (k in 1:length(M_markG1)){
for(k in 1:4 ){
  Melanoma_CC <- data.frame(matrix(NA, nrow = nrow(all_prot_M), ncol = 3))
  colnames(Melanoma_CC) <- c('G1','S','G2')
...
Plotting of protein enrichment analysis
```{r Plotting for PSEA}
## plot

#infoM_save <- infoM_save %>% dplyr::filter(FDR < .01)
#infoU_save <- infoU_save %>% dplyr::filter(FDR < .01)
infoM_save$FDR <- NULL
infoM_save$blank <- 0
infoU_save$FDR <- NULL
...
top20 <- PSEA_full[hold_names_top,]#[c(1,2,3,6,7,8,11,16,18,20,21,24,27,29,28,31,30,37,34,33,41,42,60,54,53,51),]
top20$diff <- NULL
...
rownames(bot20)
bot20 <- PSEA_full[(nrow(PSEA_full)-60):(nrow(PSEA_full)-0),]#[c((nrow(PSEA_full)-20):(nrow(PSEA_full)),157,139),]
bot20$diff <- NULL
...
t20 <- Heatmap(top20_2,cluster_columns = F,cluster_rows = F,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )
t20
...
# 
# ## Plotting Box Plots
# unique_GO_terms <- c("MHC class II protein complex binding","proteasome core complex")
...
# b20
# rownames(top20_2)
# top20_2 <- top20_2[c(3:9,16,17,15,10,11,18,12:14,2,1),]
```

```{r CDC validate correlations with sub sampling}

## gathering names of Go terms to plot 

hold_names_bot <- rownames(bot20_2)[c(1:5,8:9,12:13,15,18,20,21)]

hold_names_bot2 <- c(hold_names_bot,'RAGE receptor binding','oligosaccharyltransferase complex',
                     'positive regulation of fibroblast proliferation',
                     'ADP binding','natural killer cell mediated cytotoxicity')

hold_names_top <- c('DNA-dependent DNA replication initiation','MCM complex','melanosome','nucleosome',
                    'DNA replication-independent nucleosome assembly','chromatin organization',
                    'alternative nuclear mRNA splicing, via spliceosome','cellular response to exogenous dsRNA',
                    'positive regulation of ubiquitin-protein ligase activity involved in mitotic cell cycle',
                    'positive regulation of mitotic cell cycle','tumor necrosis factor-mediated signaling pathway',
                    'RNA catabolic process','proteasome core complex','threonine-type endopeptidase activity',
                    'DNA damage response, signal transduction by p53 class mediator resulting in cell cycle arrest',
                    'response to endoplasmic reticulum stress','activation of signaling protein activity involved in unfolded protein response','protein N-linked glycosylation via asparagine')




unique_GO_terms2 <- unique(hold_names_bot2)# c(rownames(top20_2))
unique_GO_terms2<- rownames(top20_2)
#store correlations for all Go terms that meet criteria 
#for (k in 1:length(M_markG1)){

subset_monocyte1 <- sample(1:ncol(t7_Monocytes), floor(ncol(t7_Monocytes)/2))
subset_monocyte2 <- (1:ncol(t7_Monocytes))[!(1:ncol(t7_Monocytes))%in%subset_monocyte1]
subset_melanoma1 <- sample(1:ncol(t7_Melanomas), floor(ncol(t7_Melanomas)/2))
subset_melanoma2 <- (1:ncol(t7_Melanomas))[!(1:ncol(t7_Melanomas))%in%subset_melanoma1]

all_prot_U1 <- all_prot_U[subset_monocyte1,]
all_prot_U2 <- all_prot_U[subset_monocyte2,]
all_prot_M1 <- all_prot_M[subset_melanoma1,]
all_prot_M2 <- all_prot_M[subset_melanoma2,]

Melanoma_CC1 <- data.frame(matrix(NA, nrow = nrow(all_prot_M1), ncol = 3))
colnames(Melanoma_CC1) <- c('G1','S','G2')
Melanoma_CC1$G1 <- M_markG1[[1]][subset_melanoma1]
Melanoma_CC1$S <- M_markS[[1]][subset_melanoma1]
Melanoma_CC1$G2 <- M_markG2[[1]][subset_melanoma1]
Melanoma_CC2 <- data.frame(matrix(NA, nrow = nrow(all_prot_M2), ncol = 3))
colnames(Melanoma_CC2) <- c('G1','S','G2')
Melanoma_CC2$G1 <- M_markG1[[1]][subset_melanoma2]
Melanoma_CC2$S <- M_markS[[1]][subset_melanoma2]
Melanoma_CC2$G2 <- M_markG2[[1]][subset_melanoma2]



Monocyte_CC1 <- data.frame(matrix(NA, nrow = nrow(all_prot_U1), ncol = 3))
colnames(Monocyte_CC1) <- c('G1','S','G2')
Monocyte_CC1$G1 <- U_markG1[[1]][subset_monocyte1]
Monocyte_CC1$S <- U_markS[[1]][subset_monocyte1]
Monocyte_CC1$G2 <-U_markG2[[1]][subset_monocyte1]
Monocyte_CC2 <- data.frame(matrix(NA, nrow = nrow(all_prot_U2), ncol = 3))
colnames(Monocyte_CC2) <- c('G1','S','G2')
Monocyte_CC2$G1 <- U_markG1[[1]][subset_monocyte2]
Monocyte_CC2$S <- U_markS[[1]][subset_monocyte2]
Monocyte_CC2$G2 <-U_markG2[[1]][subset_monocyte2]




### 
df_cc_cors_M1 <- data.frame(matrix(NA, nrow = (ncol(all_prot_M1)), ncol = 4))
colnames(df_cc_cors_M1) <- c('prot','G1','S','G2')
for (i in 1:(ncol(all_prot_M1))){
  df_cc_cors_M1$prot[i] <- colnames(all_prot_M1[i])
  df_cc_cors_M1$G1[i] <- cor(Melanoma_CC1$G1,all_prot_M1[i],use = 'pairwise.complete.obs')
  df_cc_cors_M1$S[i] <- cor(Melanoma_CC1$S,all_prot_M1[i],use = 'pairwise.complete.obs')
  df_cc_cors_M1$G2[i] <- cor(Melanoma_CC1$G2,all_prot_M1[i],use = 'pairwise.complete.obs')
  
}
df_cc_cors_M1[is.na(df_cc_cors_M1)==T] = 0 

df_cc_cors_M2 <- data.frame(matrix(NA, nrow = (ncol(all_prot_M2)), ncol = 4))
colnames(df_cc_cors_M2) <- c('prot','G1','S','G2')
for (i in 1:(ncol(all_prot_M2))){
  df_cc_cors_M2$prot[i] <- colnames(all_prot_M2[i])
  df_cc_cors_M2$G1[i] <- cor(Melanoma_CC2$G1,all_prot_M2[i],use = 'pairwise.complete.obs')
  df_cc_cors_M2$S[i] <- cor(Melanoma_CC2$S,all_prot_M2[i],use = 'pairwise.complete.obs')
  df_cc_cors_M2$G2[i] <- cor(Melanoma_CC2$G2,all_prot_M2[i],use = 'pairwise.complete.obs')
  
}
df_cc_cors_M2[is.na(df_cc_cors_M2)==T] = 0 


df_cc_cors_U1 <- data.frame(matrix(NA, nrow = ncol(all_prot_U1), ncol = 4))
colnames(df_cc_cors_U1) <- c('prot','G1','S','G2')
for (i in 1:ncol(all_prot_U1)){
  df_cc_cors_U1$prot[i] <- colnames(all_prot_U1[i])
  df_cc_cors_U1$G1[i] <- cor(Monocyte_CC1$G1,all_prot_U1[i],use = 'pairwise.complete.obs')
  df_cc_cors_U1$S[i] <- cor(Monocyte_CC1$S,all_prot_U1[i],use = 'pairwise.complete.obs')
  df_cc_cors_U1$G2[i] <- cor(Monocyte_CC1$G2,all_prot_U1[i],use = 'pairwise.complete.obs')
}
df_cc_cors_U1[is.na(df_cc_cors_U1)==T] = 0 

df_cc_cors_U2 <- data.frame(matrix(NA, nrow = ncol(all_prot_U2), ncol = 4))
colnames(df_cc_cors_U2) <- c('prot','G1','S','G2')
for (i in 1:ncol(all_prot_U2)){
  df_cc_cors_U2$prot[i] <- colnames(all_prot_U2[i])
  df_cc_cors_U2$G1[i] <- cor(Monocyte_CC2$G1,all_prot_U2[i],use = 'pairwise.complete.obs')
  df_cc_cors_U2$S[i] <- cor(Monocyte_CC2$S,all_prot_U2[i],use = 'pairwise.complete.obs')
  df_cc_cors_U2$G2[i] <- cor(Monocyte_CC2$G2,all_prot_U2[i],use = 'pairwise.complete.obs')
}
df_cc_cors_U2[is.na(df_cc_cors_U1)==T] = 0 


infoM <- data.frame(matrix(NA, nrow = length(unique_GO_terms2), ncol = 6 ))
infoU <- data.frame(matrix(NA, nrow = length(unique_GO_terms2), ncol = 6 ))
colnames(infoM) <- c('G1_M1','G1_M2','S_M1','S_M2','G2_M1','G2_M2')
colnames(infoU) <- c('G1_U1','G1_U2','S_U1','S_U2','G2_U1','G2_U2')


for (i in 1:length(unique_GO_terms2)){
  GO_db_lim <- GO_db %>% dplyr::filter(GO_term_name == unique_GO_terms2[i])
  df_M1 <- df_cc_cors_M1 %>% dplyr::filter(prot %in% GO_db_lim$Uniprot)
  df_M1$prot <- NULL
  df_M2 <- df_cc_cors_M2 %>% dplyr::filter(prot %in% GO_db_lim$Uniprot)
  df_M2$prot <- NULL
  df_U1 <- df_cc_cors_U1 %>% dplyr::filter(prot %in% GO_db_lim$Uniprot)
  df_U1$prot <- NULL
  df_U2 <- df_cc_cors_U2 %>% dplyr::filter(prot %in% GO_db_lim$Uniprot)
  df_U2$prot <- NULL
  
  if(unique_GO_terms2[i]=='RNA catabolic process'){
    df_M1_save <- df_M1
    df_M2_save <- df_M2
    df_U1_save <- df_U1
    df_U2_save <- df_U2
  ...
  infoM[i,c(1,3,5)]<- colMedians(as.matrix(df_M1),na.rm = T)
  infoM[i,c(2,4,6)]<- colMedians(as.matrix(df_M2),na.rm = T)
  
  infoU[i,c(1,3,5)]<- colMedians(as.matrix(df_U1),na.rm = T)
  infoU[i,c(2,4,6)]<- colMedians(as.matrix(df_U2),na.rm = T)
  
  rownames(infoM)[i] <- unique_GO_terms2[i]
  rownames(infoU)[i] <- unique_GO_terms2[i]

}


#Heatmap(infoM[rownames(top20_2),],cluster_columns = F,cluster_rows = F,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )

#Heatmap(infoU[rownames(top20_2),],cluster_columns = F,cluster_rows = F,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )
unique_GO_terms2
Mel_sim_map <- Heatmap(infoM[unique_GO_terms2,],cluster_columns = F,cluster_rows = F,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )

Mon_sim_map <- Heatmap(infoU[unique_GO_terms2,],cluster_columns = F,cluster_rows = F,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )

cor(c(infoU[,1],infoU[,3],infoU[,5]),c(infoU[,2],infoU[,4],infoU[,6]))
cor.test(c(infoU[,1],infoU[,3],infoU[,5]),c(infoU[,2],infoU[,4],infoU[,6]))
view(infoU)
colnames(df_M1_save)[3] <-"zG2"
colnames(df_M2_save)[3] <-"zG2"
colnames(df_U1_save)[3] <-"zG2"
colnames(df_U2_save)[3] <-"zG2"

df_M1_save <- melt(df_M1_save)
df_M1_save$variable_new <- paste0(df_M1_save$variable,'_1')
df_M2_save <- melt(df_M2_save)
df_M2_save$variable_new <- paste0(df_M2_save$variable,'_2')
df_U1_save <- melt(df_U1_save)
df_U1_save$variable_new <- paste0(df_U1_save$variable,'_1')
df_U2_save <- melt(df_U2_save)
df_U2_save$variable_new <- paste0(df_U2_save$variable,'_2')

df_U_save <- rbind(df_U1_save,df_U2_save)
df_M_save <- rbind(df_M1_save,df_M2_save)

U_box_sim <- ggplot(df_U_save,aes(x = variable_new, y = value, color =variable )) +geom_boxplot(lwd=1.5,)+scale_color_manual(values=c("#FC7E00","#794119", "#E167BF"))+
  theme_bw()+ggtitle('RNA catabolism') +
  ylab('') + xlab('')+ coord_cartesian(ylim = c(-.25,.2))+
  theme(panel.border = element_rect(fill=NA, colour = "black", size=2))+
  theme(axis.text.y = element_text(size = 40,color = 'black'), axis.title=element_text(size=30))+
  rremove('legend')

M_box_sim <- ggplot(df_M_save,aes(x = variable_new, y = value, color =variable )) +geom_boxplot(lwd=1.5,)+scale_color_manual(values=c("#FC7E00","#794119", "#E167BF"))+
  theme_bw()+ggtitle('RNA catabolism') +
  ylab('') + xlab('')+ coord_cartesian(ylim = c(-.25,.2))+
  theme(panel.border = element_rect(fill=NA, colour = "black", size=2))+
  theme(axis.text.y = element_text(size = 40,color = 'black'), axis.title=element_text(size=30))+
...
ggsave('/Users/andrewleduc/Desktop/nPOP_Paper/PDFs/CDC/CDCBox_Mel_sim.pdf',plot = M_box_sim,height = unit(6, "in"),width = unit(7.5, "in"))
ggsave('/Users/andrewleduc/Desktop/nPOP_Paper/PDFs/CDC/CDCBox_Mon_sim.pdf',plot = U_box_sim,height = unit(6, "in"),width = unit(7.5, "in"))
pdf('/Users/andrewleduc/Desktop/CDC_map_Mel_Sim.pdf',width=6,height=3.25)
Mel_sim_map
dev.off()
pdf('/Users/andrewleduc/Desktop/CDC_map_Mon_Sim.pdf',width=6,height=3.25)
Mon_sim_map
dev.off()
```


Projecting proteins into a joint space based off proteins that make up markers and color coding by abundance of protiens from different phases
```{r PCA on CDC Proteins}

Mark_all <- c(S_mark1,S_mark2,G2_mark1,G2_mark2,G1_mark1,G1_mark2)

Melanoma_CC <- t7_Melanomas %>% dplyr::filter(rownames(t7_Melanomas) %in% Mark_all)
...
M_na <- colMeans(Melanoma_CC,na.rm = T)
Melanoma_CC <- Melanoma_CC[,(is.na(M_na)==F)]
Mon_na <- colMeans(Monocyte_CC,na.rm = T)
Monocyte_CC <- Monocyte_CC[,(is.na(Mon_na)==F)]

Melanoma_CC <- filt.mat.cr(cr_norm_log(as.matrix(Melanoma_CC)),.9,.9)
Monocyte_CC <- filt.mat.cr(cr_norm_log(as.matrix(Monocyte_CC)),.9,.9)


Melanoma_CC <- as.data.frame(Melanoma_CC)
Monocyte_CC <- as.data.frame(Monocyte_CC)
Melanoma_CC <- Melanoma_CC %>% dplyr::filter(rownames(Melanoma_CC) %in% rownames(Monocyte_CC))

X.m <- cbind(Melanoma_CC,Monocyte_CC)
...
Melanoma_CC[Melanoma_CC==0] <- NA
Melanoma_CC <- as.data.frame(Melanoma_CC)
Monocyte_CC[Monocyte_CC==0] <- NA
Monocyte_CC <- as.data.frame(Monocyte_CC)

#HeLa average protein abundance vectors
SH <-  Melanoma_CC %>% dplyr::filter(rownames(Melanoma_CC) %in% S)
SH <- as.matrix(SH)
G1H <- Melanoma_CC %>% dplyr::filter(rownames(Melanoma_CC) %in% G1)
G1H <- as.matrix(G1H)
G2H <- Melanoma_CC %>% dplyr::filter(rownames(Melanoma_CC) %in% G2)
G2H <- as.matrix(G2H)
...
#ggsave('/Users/andrewleduc/Desktop/nPOP_Paper/PDFs/CDC/G1.pdf',plot = G1_pca_plot,width = unit(6.5, "cm"),height =  unit(5, "cm"))
#ggsave('/Users/andrewleduc/Desktop/nPOP_Paper/PDFs/CDC/S.pdf',plot = S_pca_plot,width = unit(6.5, "cm"),height =  unit(5, "cm"))
#ggsave('/Users/andrewleduc/Desktop/nPOP_Paper/PDFs/CDC/G2.pdf',plot = G2_pca_plot,width = unit(6.5, "cm"),height =  unit(5, "cm"))
...
Here we implement a graph matching algorithm that takes a greedy approach to assigning cells to a given CDC based off abundance of different makers. Distribution of cells overall is forced to match that from the bulk data, since we have a prior that tells us the amount of cells in G1, S and G2. This analysis gives us a way to compare the relative distributions for each sub population of melanoma cells (which is not differentially effected by predetermining amount of cells overall that will be in each phase).
```{r Graph match Greedy Approach}
CPC_f_test <- CPC_f %>% dplyr::select(Id,G1_p,S_p,G2_p)
...
Plotting for CDC greedy assignments
...
dist <- ggplot(Mel_cc_plot,aes(y = Pct,x =Phase, fill = Cluster)) + 
  ...
  theme_classic() + scale_fill_manual(values = c("#dcdcdc", "#808080"))+
...
dist
ggsave('/Users/andrewleduc/Desktop/nPOP_Paper/PDFs/CDC_dist.pdf',plot = dist,height = unit(4, "cm"),width = unit(5, "cm"))



```

Analysis of protein set enrichment as done above but on the two subpopulations of melanoma cells. We sample from the larger population for computing correlations since # of cells in distribution will effect magnitude of the correlations
```{r Protein Set Enrichment analysis Melanoma subPop}
...
all_prot_M <- filt.mat.cr(as.matrix(t7_Melanomas),.9,.99)
t7_Melanomas_Large_filt <- as.data.frame(all_prot_M) %>% dplyr::select(meta_mel_C1$id)
t7_Melanomas_Small_filt <- as.data.frame(all_prot_M) %>% dplyr::select(meta_mel_C2$id)

all_prot_U <- as.data.frame(t(t7_Melanomas_Small_filt))


all_prot_U_G1 <- all_prot_U %>% dplyr::select(c(G1_mark1[1:2],G1_mark2[1:2]))
all_prot_U_S <- all_prot_U %>% dplyr::select(c(S_mark1[1:2],S_mark2[1:2]))
...
df_cc_cors_U[is.na(df_cc_cors_U)==T] = 0
nrow(df_cc_cors_U)
df_cc_cors_U <- df_cc_cors_U[rowSums(df_cc_cors_U[,2:4])!=0,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,2])!=1,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,3])!=1,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,4])!=1,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,2])!=-1,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,3])!=-1,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,4])!=-1,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,2])!=0,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,3])!=0,]
df_cc_cors_U <- df_cc_cors_U[(df_cc_cors_U[,4])!=0,]
...
df_cc_cors_M$prot <- rownames(all_prot_M)
...
for(i in 1:11){
  ...
  all_prot_M_S <- all_prot_M %>% dplyr::select(c(S_mark1[1:2],S_mark2[1:2]))
...
df_cc_cors_M <- df_cc_cors_M %>% filter(prot %in% df_cc_cors_U$prot)

Heatmap(cor(df_cc_cors_M[,(2:ncol(df_cc_cors_M))],use = 'pairwise.complete.obs'))
...
for (i in 1:length(unique_GO_terms)){
  ...
  if (nrow(df_temp) > 2){
    if (nrow(df_temp) < 100){
      ...
      infoM[count,1]<- vect[1]
      infoM[count,2]<- vect[2]
      infoM[count,3]<- vect[3]
...
infoU <- infoU %>% filter(fdrnew < .01)
infoM <- infoM %>% filter(fdrnew < .01)
...
PSEA_full__ <- PSEA_full_[1:15,]

spa <- Heatmap(PSEA_full__,cluster_columns = F,cluster_rows = T,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8), show_row_dend = FALSE )
            
PSEA_full_2 <- PSEA_full_[row_order(spa),]

nrow(PSEA_full)
new_heat <- Heatmap(PSEA_full[terms,],cluster_columns = F,cluster_rows = T,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8), show_row_dend = FALSE )

Heat_hold <- PSEA_full[terms,]
Heat_hold <- Heat_hold[row_order(new_heat),]
write(rownames(Heat_hold),'/Users/andrewleduc/Desktop/smallvLarge.txt')
  
terms <-c('glycolysis','epithelial cell differentiation','glucose 6-phosphate metabolic process','chromosome','microtubule associated complex','small ribosomal subunit','actomyosin structure organization','recycling endosome membrane','nucleosome','GDP binding','tricarboxylic acid cycle','histone phosphorylation','protein neddylation','cellular response to mechanical stimulus','glutamine metabolic process')
```


Here we compute a protein set enrichment analysis between the bulk DIA data as validation of CDC fraction data that we used to find cell cycle proteins
```{r Bulk Diff protein analysis/sc validation}

#collapse to protein level by median peptide abundance
...
ev_both <- ev_both %>% filter(Protein %in% colnames(all_prot_M))

#Protien set enrichment analysis (function in functions script)
Bulk_GSEA <- Population_GSEA_norm(ev_both,GO_db)
Bulk_GSEA <- Bulk_GSEA %>% filter(is.na(GO_term)==F)
Bulk_GSEA$FDR <- p.adjust(Bulk_GSEA$pVal,method = 'BH')
Bulk_GSEA <- Bulk_GSEA %>% filter(FDR < .05)
colnames(Bulk_GSEA)[5:7] <- c('G1','S','G2')
Bulk_GSEA <- Bulk_GSEA[order(Bulk_GSEA$pVal),]

Bulk_GSEA_sect <- Bulk_GSEA %>% filter(GO_term %in% hold_names_top)
colnames(Bulk_GSEA_sect)[5:7] <- c('G1','S','G2')
rownames(Bulk_GSEA_sect) <- Bulk_GSEA_sect$GO_term
Bulk_GSEA_sect <- Bulk_GSEA_sect %>% select(G1,S,G2)
Bulk_GSEA_sect <- Bulk_GSEA_sect[order(rownames(Bulk_GSEA_sect)),]
top20_2_sect <- top20_2[rownames(Bulk_GSEA_sect),]

t20_bulk <- Heatmap(top20_2_sect,cluster_columns = F,cluster_rows = T,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )

t20_bulk
Bulk_GSEA_sect <- Bulk_GSEA_sect[row_order(t20_bulk),]
Heatmap(Bulk_GSEA_sect,cluster_columns = F,cluster_rows = F,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )


Bulk_GSEA_sect.m <- melt(as.matrix(Bulk_GSEA_sect)[rownames(top20_2_sect),])
top20_2_sect.m1 <- melt(top20_2_sect[,1:3])
top20_2_sect.m2 <- melt(top20_2_sect[,5:7])
top20_2_sect.med <- c(top20_2_sect.m2$value,top20_2_sect.m1$value)

Bulk_GSEA_sect.m$value <- (Bulk_GSEA_sect.m$value-mean(Bulk_GSEA_sect.m$value))/sd(Bulk_GSEA_sect.m$value)
top20_2_sect.med <- (top20_2_sect.med-mean(top20_2_sect.med))/sd(top20_2_sect.med)

cor(top20_2_sect.med,c(Bulk_GSEA_sect.m$value,Bulk_GSEA_sect.m$value))
cor.test(top20_2_sect.med,c(Bulk_GSEA_sect.m$value,Bulk_GSEA_sect.m$value),method = "spearman",exact=FALSE)$p.value

c('P35908','Q16763','Q9ULW0','Q02241','P49454','P53350','Q9BW19','Q14807')
c('Q15004','P12004','Q9BS18')
diff_prot_output_filt <- diff_prot_output_filt[order(-diff_prot_output_filt$G1),]
prots_diff <- c('P35908','Q16763','Q9ULW0','Q02241','P49454','P53350','Q9BW19','Q14807',
                'Q15004','P12004','Q9BS18','A6NDG6','P04264',diff_prot_output_filt$Protein[1:10])

diff_prot_output_filt_plot <- diff_prot_output_filt %>% filter(Protein %in% prots_diff)
diff_prot_output_filt_plot <- diff_prot_output_filt_plot %>% left_join(convert, by= c('Protein'='Entry'))
diff_prot_output_filt_plot <- diff_prot_output_filt_plot %>% select(Protein.names,G1,S,G2)
rownames(diff_prot_output_filt_plot) <- diff_prot_output_filt_plot$Protein.names
diff_prot_output_filt_plot$Protein.names <- NULL
diff_prot_output_filt_plot[diff_prot_output_filt_plot >1] <- 1
diff_prot_output_filt_plot[diff_prot_output_filt_plot < -1] <- -1
diff_prot_output_filt_plot<- as.matrix(diff_prot_output_filt_plot)
diff_prot_plot <- Heatmap(diff_prot_output_filt_plot,cluster_columns = F,cluster_rows = F,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE ) 

diff_prot_output_filt_plot <- diff_prot_output_filt_plot[row_order(diff_prot_plot),]
diff_prot_output_filt_plot <- diff_prot_output_filt_plot[c(1:13,16,19:21,23,14,15,17,18,22),]

write(rownames(diff_prot_output_filt_plot),'/Users/andrewleduc/Desktop/prots.txt')
write(rownames(Bulk_GSEA_sect),'/Users/andrewleduc/Desktop/prot_set.txt')
write(rownames(top20_2),'/Users/andrewleduc/Desktop/prot_setsim.txt')
write(unique_GO_terms2,'/Users/andrewleduc/Desktop/prot_setdif.txt')
...
intersect(gene_names$hgnc_symbol,diff_prot_output_filt_plot$Gene.names2)
