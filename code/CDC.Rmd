...
G2_df$Protein <- G2_df %>% filter(Protein %in% good_prot)
...
Here we compute a protein set enrichment analysis between the bulk DIA data as validation of CDDC fraction data that we used to find cell cycle proteins
```{r Bulk GSEA}
...
for(i in 1:(nrow(mG1)/2)){
  ...
  for (j in 1:(nrow(mS)/2)){
    ...
    for (k in 1:(nrow(mG2)/2)){
      ...
      #s3_hold <-c(s3_hold,s3)
      
      if((s1 < -.046) & (s2 < -.046) & (s3 < -.048)){
          i_t <- c(i_t,i)
          j_t <- c(j_t,j)
          k_t <- c(k_t,k)
      }
...
for (i in 1:nrow(t7_Monocytes)){
  ...
  CDC_prot_disco_mon[count,3] <- cor.test(t(t7_Monocytes[i,]),U_markG2[[20]],method = "spearman",exact=FALSE)$p.value
...
all_go_count <- all_go_count %>% filter(V2 > 50)
...
top20 <- PSEA_full[1:30,]#[c(1,2,3,6,7,8,11,16,18,20,21,24,27,29,28,31,30,37,34,33,41,42,60,54,53,51),]
top20$diff <- NULL
...
t20 <- Heatmap(top20,cluster_columns = F,cluster_rows = T,show_heatmap_legend = T,row_names_gp = grid::gpar(fontsize = 8),show_row_dend = FALSE )
