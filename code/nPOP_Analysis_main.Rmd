...
ev <- read.delim("../dat/raw_data/ev_updated.txt")
...
neg_miss <- (colSums(ev.matrix.neg==0,na.rm=T)/colSums(is.na(ev.matrix.neg)==F))
missingDF <- as.data.frame(neg_miss)
missingDF$cond <- 'neg'
colnames(scDF.m)[1] <- 'fract.miss'


sc_miss.m <- (colSums(ev.matrix.sc.m==0,na.rm=T)/colSums(is.na(ev.matrix.sc.m)==F))
scDF.m<- as.data.frame(sc_miss.m)
scDF.u$cond <- 'mon'
df_all <- rbind(missingDF,scDF.u,scDF.m)

ggplot(df_all,aes(x =fract.miss,fill = cond )) + geom_histogram(position = 'identity',alpha = .3)
ggplot(df_all,aes(x =fract.miss,fill = cond )) + geom_density(alpha = .3)+theme_classic()
...
pca_main <- pca.display %>% ggplot(aes(x = PC1, y = PC2, color = type_col, shape = type_sh, size = type_sh,alpha = type_sh))  + 
  ...
  xlim(c(-.035,.035)) + xlab('PC1 (62%)') + ylab('PC2 (3%)')


ggsave("/Users/andrewleduc/Desktop/nPOP_Paper/figs/SC_QC/pca_main.png",plot = pca_main,width = unit(6, "cm"),height =  unit(5, "cm"))
